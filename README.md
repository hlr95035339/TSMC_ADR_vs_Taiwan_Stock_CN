# 台積電 ADR vs 台股價差分析（TSMC ADR vs TWSE）

## 專案簡介
**為何台積電 ADR（NYSE: TSM）長期相較台股（TWSE: 2330）存在溢價？此價差是否具有可交易性？**

本專案分析台積電 ADR 與台股之間的價格差異（spread），並探討其成因與是否存在可行的交易策略（pair trading / mean reversion）。
